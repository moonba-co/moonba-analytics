  ...
  const ListItem = ({ item }) => {
    return (
      <DashGrid style={{ height: '48px' }}>
        ...
        <DataText area="value">
          {currency === 'OLT' ? 'Ξ ' + formattedNum(item.valueETH) : formattedNum(item.amountUSD, true)}
        </DataText>
        {!below780 && (
          <>
        ...
        {!below1080 && (
          <DataText area="account">
            <Link color={color} external href={'https://mainnet-explorer.oneledger.network/address/' + item.account}>
